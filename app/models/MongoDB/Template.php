...
class Template extends Moloquent {
	...
	private static function validateTemplate($template) {
		...
		if( is_null($template->platform) ||
			is_null($template->type) ||
			is_null($template->instructions) ||
			is_null($template->cml) ||
			is_null($template->css) ||
			is_null($template->js) ||
			is_null($template->version) ||
			is_null($template->user_id)) {
			throw new \Exception('Template does not comply with data model!'.$template);
		}
	}
