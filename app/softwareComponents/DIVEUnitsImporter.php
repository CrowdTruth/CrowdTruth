		...
		if ($settings["token"] != null && $settings["token"] != "") {
			...
			if(!$user) {
				...
				if(Auth::check()) {
					...
					if ($settings['project'] != '' && $settings['project'] != null) {
						...
						if (!in_array($settings['project'], $projectsAll)) {
							...
							if(!ProjectHandler::inGroup($user['_id'], $settings['project'])) {
								...
								if (isset($data) && !empty($data)) {
									...
									foreach ($dataContent as $unitContent) {
										// Create activity
									...
									array_push($this->status['error'], "No units were sent from the dashboard: data -- " . $data . "");
									return $this->status;
								}
							}
						}
