...
def img_to_matrix(filename, tr, STANDARD_SIZE, verbose=False):
   ...
   # img = img.enhance(0.8)
    ...
    H,S,V = HSV(img2)    
    r, g, b = img2.split()
   # H = np.array(Image.fromarray(np.array(H),'L'))
    #S = np.array(Image.fromarray(np.array(S),'L'))
   # V = np.array(Image.fromarray(np.array(V),'L'))
   # print H
   # print r
