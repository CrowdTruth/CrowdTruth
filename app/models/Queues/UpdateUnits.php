...
class UpdateUnits {
	...
	public function fire($job, $data) {
		...
		foreach ( $data as $id ) {
			...
			// dd($id);
			$batch ['count'] = count ( \MongoDB\Entity::where ( 'documentType', 'batch' )->where ( 'parents', 'all', array (
					$unit->_id 
			) )->get ()->toArray () );
			...
			// filtered
			$filteredField = array ();
			$filteredField ['job_ids'] = array_flatten ( Job::where ( 'metrics.filteredUnits.list', 'all', array (
					$unit ['_id'] 
			) )->get ( [ 
					'_id' 
			] )->toArray () );
			...
			$derivatives = \MongoDB\Entity::whereIn ( 'parents', array (
					$unit->_id 
			) )->lists ( '_id' );
